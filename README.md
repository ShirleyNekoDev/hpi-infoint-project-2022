...
topic. After that, it increments the `day` value and sends another GET request.
This process continues until `end_date` (inclusive) is reached, and the crawler will stop automatically.

### FFB trade-events topic
The `trade-events` holds all the events (announcements) produced by the `ffb_crawler`. Each message in a Kafka topic
consist of a key and value.
