...
### RB corporate-events topic

The `rb_announcements` holds all the events (announcements) produced by the `rb_crawler`. Each message in a Kafka topic
consist of a key and value.
The `rb_filtered_announcements`
The `rb_companies`
The `rb_persons`
...
### FFB topics
The `ffb_trade-events` holds all the events produced by the `ffb_crawler`. Each message in a Kafka topic
consist of a key and value.

The `ffb_filtered_trade-events`
The `ffb_companies`
...
```shell
poetry run python ffb_crawler/main.py --start-date $START_DATE --end-date $END_DATE
```

* The `--start-date` option is a date in format YYYY-MM-DD, which determines the first day of the Frankfurter Boerse trade history to be
crawled (defaults to today).
* The `--end-date` option is similar to start-date, but defines the last day to be crawled (defaults to today).
* You can use the `--help` option to recap the usage.

The crawler will crawl all days in the date range, ignoring weekends (trade free). To crawl only one day, use the same value for start and end date.
